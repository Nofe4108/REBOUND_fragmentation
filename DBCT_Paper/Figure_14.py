...
for axis in axes:
    ...
    axis.grid(alpha=0.7)
    axis.scatter(final_masses[axis_index], final_cmfs[axis_index], s=3.0, color='black')
    axis.axvline(.093, label = 'Embryo Mass', color = 'tab:orange', linestyle = '--', alpha=.7)
    axis.axvline(.0093, label = 'Planetesimal Mass', color = 'tab:green', linestyle = '--', alpha=.7)
    axis.axhline(initial_average_cmfs[axis_index], label = 'Initial Average CMF', color = 'tab:blue', linestyle = '--', alpha=.7)      

ax0.set_title('a)', fontsize='x-large'), ax0.legend(framealpha=1.0, fontsize=8.0)
ax2.set_title('b)', fontsize='x-large'), ax2.legend(framealpha=1.0, fontsize=8.0)
ax4.set_title('c)', fontsize='x-large'), ax4.legend(framealpha=1.0, fontsize=8.0)
...
plt.savefig("graphs/fig14.pdf", bbox_inches='tight', pad_inches=0.01)
plt.savefig('graphs/fig14.eps', bbox_inches='tight', pad_inches=0.01)
plt.savefig('graphs/fig14.png', bbox_inches='tight', dpi=300)