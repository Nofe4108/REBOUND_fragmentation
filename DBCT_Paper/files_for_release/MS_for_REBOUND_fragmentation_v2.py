...
def track_composition(collision_report_file, composition_input_file, ejection_file, min_core_collision_frac, max_core_collision_frac):
    ...
    for i in range(len(collisions)): 
        ...
        target_radius = float(collision[5])/5
        proj_hash = int(collision[6]) 
        proj_radius = float(collision[7])/5
        no_frags = int((len(collision)-8)/2)
        frag_hashes = [int(collision[i*2+6]) for i in range(1,no_frags+1)]
        frag_masses = [float(collision[i*2+7]) for i in range(1,no_frags+1)]
        ...
        targ_idx = [idx for idx in range(len(compositions)) if int(compositions[i][0])==target_hash][0] # Index of target in the compositions list
        proj_idx = [idx for idx in range(len(compositions)) if int(compositions[i][0])==proj_hash][0] # Index of projectile in the compositions list
        ...
        # This sequence estimates the core radii of the target and projectile
        ...
        #4 LINES BELOW THIS CAN PROBABLY BE ELIMINATED
    ...
    ############### DISRUPTIVE COLLISIONS ############### 
        ...
        for i in range(len(compositions[targ_idx])):
            if i == 1:
                if compositions[targ_idx][i] < 0.0: 
                ...
                if compositions[targ_idx][i] < 0.0:
                    ...
                    sys.exit(1)
                elif compositions[targ_idx][i] > 1.0:
...
# Input Files
collision_file = "collision_report_10Myr.txt"
comp_input_file = "disk_mantle_stripping_input.txt"
ejec_file = "ejections_10Myr.txt"

# Output Files
comp_output_file = "mantle_stripping_output_10Myr.txt"

# Mantle Stripping Model Parameters
min_ejecta_core_frac = 0.0
max_ejecta_core_frac = 1.0

# Runs composition tracking and outputs file
