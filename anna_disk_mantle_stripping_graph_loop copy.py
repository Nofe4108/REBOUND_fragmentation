    ...
    for c in comp:
        ...
        if no < 11:
            ...
            if 0.498 < c[2] < .501:
                print(c[0])
                print(no)
            if 0.99 < c[2] < 1.1 and c[1] == 1.4e-8:
                print(c[0])
                print(no)
...
for comp in compositions:
    final_comp_hashes.append(comp[0])
...
original_hashes = [original_compositions[i][0] for i in range(len(original_compositions))]

for i in range(len(final_comp_hashes)):
    io_flag = 0
    for hsh in original_hashes:
        if final_comp_hashes[i] == hsh:
            io_flag += 1
    if io_flag == 1:
        io_colors.append('tab:blue')
    else:
        io_colors.append('tab:red')

fig8, ax8 = plt.subplots(figsize=(6,5))
ax8.scatter(final_masses_comp, final_core_fracs, s=5.0, color = io_colors)
#ax1.axhline(0.3, label = 'Initial CMF', color = 'tab:blue', linestyle = '--', alpha=.7)
ax8.axvline(.093, label = 'Embryo Mass', color = 'tab:orange', linestyle = '--', alpha=.7)
ax8.axvline(.0093, label = 'Planetesimal Mass', color = 'tab:green', linestyle = '--', alpha=.7)
ax8.set_xlabel('Mass ($M_{\u2295}$)',fontsize='large')
ax8.set_ylabel("CMF", fontsize='large')
plt.xscale('log')
ax8.set_ylim([0, 1.1])
plt.yticks(np.arange(0, 1.1, .1))
ax8.minorticks_on()
