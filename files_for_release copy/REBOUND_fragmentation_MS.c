...
//0=elastic bounce, 1=merger, 2=partial accretion, 3=partial erosion, 4=supercat
    FILE* of = fopen("../disk_sim_data/collision_report.txt","a+");
    fprintf(of, "%e\t", r->t);
...
void heartbeat(struct reb_simulation* sim){
    if (reb_output_check(sim, 1000)){   //heartbeat is at every 100 years
...
int main(int argc, char* argv[]){
    ...
    int no_embryos = 26;
    double m_mars = 0.093/332954.355179; //mass of moon in units of Msun (Mearth -> Msun)
    
    for (int i=0;i<no_embryos;i++){
        ...
        p.r = expansion_factor*get_radii(p.m,rho);
        char hash[10];
    ...
    int no_planetesimals = 260;
    double m_moon = 0.0093/332954.355179; //mass of moon in units of Msun (Mearth -> Msun)
    
    for (int i=0;i<no_planetesimals;i++){
    double y = reb_random_uniform(sim,0,1.0);
    double semi = 3.65*pow(y,1./.5)+.35;
    ...
    p.r = expansion_factor*get_radii(p.m,rho);
    char hash[10];
    sprintf(hash,"PL%d", i+1+no_embryos);
    p.hash = reb_hash(hash);
    reb_add(sim,p);
    ...
    reb_simulationarchive_automate_interval(sim,filename,sim_archive_time);
    reb_integrate(sim, sim_time);
