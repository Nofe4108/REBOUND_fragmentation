...
int hit_and_run(struct reb_simulation* const r, struct reb_collision c, struct collision_params *params){  //also includes partial accretion.  Mlr = M_target.  Projectile is erroded.
        ...
        if (params->Vi <= v_crit){             //if impact velocity is low, the hit-and-run results in a merger.
            ...
            if (new_projectile_mass-Mlr_dag < min_frag_mass){
                ...
                params->Mslr = Mlr_dag; //FIXME: why is this not new_projectile_mass?
                printf("HIT AND RUN\n");
...
//0=elastic bounce, 1=merger, 2=partial accretion, 3=partial erosion, 4=supercat
    FILE* of = fopen("/Users/nferich/GitHub/REBOUND_fragmentation/mantle_stripping/collision_report.txt","a+");
    fprintf(of, "%e\t", r->t);
...
int reb_collision_resolve_fragment(struct reb_simulation* const r, struct reb_collision c){
    ...
    if (G==39.476926421373 || G==1){rho1 = 1.684e6;}  //Msun/AU^3 //FIXME: The way densities are resolved in here might cause issues with the calculation of radii for the mantle stripping code
...
///POPULATE STRUCT OBJECTS
    ...
    if (Vi <= V_esc){
        ...
        if (M_tot - params->Mlr < min_frag_mass){
            ...
            merge(r,c,params);
            print_collision_array(r,c,params, targ_m, imp_r);
            return swap;
                                             }
            ...
            //FIXME: If projectile is partioned based on b, new code here
...
const double core_density = 7874.0; //kg m^-3 - density of iron
const double mantle_density = 3330.0; //kg m^3 -  approximate density of Earth's upper mantle

double get_radius(double mass, double core_frac, double mantle_frac){
    
    double core_mass = core_frac*mass;
    double mantle_mass = mantle_frac*mass;
    
    double core_radius = pow((3*mass)/(4*M_PI*mantle_density), 1./3.);
    double radius = pow(pow(core_radius, 3.)+((3*mantle_mass)/(4*M_PI*mantle_density)), 1./3.);
...
int main(int argc, char* argv[]){
    ...
    double test_radius = get_radius(1., .3, .7);
    printf("%1.2f", test_radius);
    ...
    //Projectile that will hit and run with target - min frag mass .001
    {
        struct reb_particle hr_body = {0};
        hr_body.x  = -2; hr_body.y  = -0; hr_body.z  = 0;
        hr_body.vx = 15.0; hr_body.vy = 9.0;
        hr_body.m  = .2;
        hr_body.r  = 0.2;
        //reb_add(sim, hr_body);
    }
