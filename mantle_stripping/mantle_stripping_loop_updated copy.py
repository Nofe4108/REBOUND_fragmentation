...
# -*- coding: utf-8 -*-
"""
Created on Mon May  1 16:18:13 2023
...
for max_cmf in max_core_collision_fracs:
    ...
    min_fracs = []
    max_fracs = []
    for min_cmf in min_core_collision_fracs:
        final_embryo_composition = []
        if max_cmf >= min_cmf: #ensures the max collision frac is never less than the minimum
                for i in file_range:
                    ...
                    final_composition = track_composition(collision_file, comp_input_file, impact_param_file, ejec_file, min_cmf, max_cmf)
                ...
                average_fracs.append(np.nan)
                min_fracs.append(np.nan)
                max_fracs.append(np.nan)
...
plot3 = ax3.imshow(max_final_core_fracs, cmap=color_map, vmin=min_final_frac, vmax=max_final_frac)
ax1.set_xticks(np.arange(len(fmin)), labels=fmin)
ax1.set_yticks(np.arange(len(fmax)), labels=fmax)
ax2.set_xticks(np.arange(len(fmin)), labels=fmin)
ax2.set_yticks(np.arange(len(fmax)), labels=fmax)
ax3.set_xticks(np.arange(len(fmin)), labels=fmin)
ax3.set_yticks(np.arange(len(fmax)), labels=fmax)
ax1.set_title('Average Final CMFs', fontsize='x-large')
ax2.set_title('Smallest Final CMFs', fontsize='x-large')
ax3.set_title('Largest Final CMFs', fontsize='x-large')
ax1.set_xlabel('f$_{min}$', fontsize='x-large', labelpad=10.0)
...
ax3.scatter(B_list, ideal_ejecta_CMF, color=collision_types, s=1.0)
...
legend = plt.legend(handles=ax3_legend_elements, loc = 'upper right', framealpha = .7)
plt.gca().add_artist(legend)

plt.savefig('graphs/BvsidealCMF.pdf', bbox_inches='tight', pad_inches=0.01)
...
ax4.scatter(B_list, actual_ejecta_CMF, color=collision_types, s=1.0)
plt.xlabel('B/$R_{targ}$', fontsize='large')
